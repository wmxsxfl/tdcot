...
def score_video(question, triplet, frame_paths, processor, model, yes_ids, no_ids, early_exit=0.3):
    # 2-state decomposition ablation.
    # Reuse the original 3-state decomposition, but verify only initial and end.
    state_specs = [
        ("initial", triplet["initial"]),
        ("end", triplet["end"]),
    ]

    mid = max(1, len(frame_paths) // 2)
    segments = {
        "initial": frame_paths[:mid],
        "end": frame_paths[mid:] or frame_paths[-1:],
    }
