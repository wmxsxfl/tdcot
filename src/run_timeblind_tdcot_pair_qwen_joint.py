...
def score_video(question, triplet, frame_paths, processor, model, yes_ids, no_ids, early_exit=0.3):
    joint_state = (
        "The video supports the full temporal sequence: "
        f"initially, {triplet.get('initial', '')}; "
        f"then, {triplet.get('transition', '')}; "
        f"finally, {triplet.get('end', '')}."
    )
