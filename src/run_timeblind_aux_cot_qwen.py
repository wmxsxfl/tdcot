...
def generate_text_hint(question, tok, model, max_new_tokens=96):
    messages = [
        {
            "role": "system",
            "content": "You generate concise temporal reasoning hints for video QA. You do not see the video.",
        },
        {
            "role": "user",
            "content": (
                "Given only the question, write a short reasoning plan that identifies what temporal evidence "
                "a video model should check. Do not answer yes or no.\n\n"
                f"Question: {question}\n\n"
                "Hint:"
            ),
        },
...
def main():
    ...
    with open(out_path, "w", encoding="utf-8") as fout:
        for x in tqdm(rows, desc="TimeBlind Auxiliary-CoT"):
