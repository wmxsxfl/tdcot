...
def score_video(question, triplet, frame_paths, processor, model, yes_ids, no_ids, early_exit=-1.0):
    segments = {
        "initial": frame_paths[0:5],
        "transition": frame_paths[5:10],
        "end": frame_paths[10:16],
    }
    probs = []
    previous = []
    for name in ["initial", "transition", "end"]:
        state = triplet[name]
        p = yes_probability(
    ...
    score = probs[0]["p_yes"] * probs[1]["p_yes"] * probs[2]["p_yes"]
...
def main():
    ...
    ap.add_argument("--early-exit", type=float, default=-1.0, help="Early exit product threshold.")
    ap.add_argument("--cache-dir", default="outputs/frame_cache")
    ap.add_argument("--limit-pairs", type=int, default=5)
    args = ap.parse_args()
