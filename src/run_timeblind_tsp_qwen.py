...
def main():
    ...
    with open(out_path, "w", encoding="utf-8") as fout:
        for x in tqdm(rows, desc="TimeBlind TSP"):
            video_path = resolve_video_path(args.data_root, x["video_path"])

            prompt = f"""Answer the video question by focusing specifically on temporal direction.

Pay attention to:
- what is visible at the beginning,
- what changes during the middle,
- what is true at the end,
- whether the event order matches the question.

Do not answer only from static object presence. Use the direction of change in the video.
