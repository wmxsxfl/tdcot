...
def parse_letter(text, n_options):
    text = str(text)
    m = re.search(r"answer\s*[:\-]?\s*([A-E])", text, flags=re.I)
    if not m:
        m = re.search(r"\b([A-E])\b", text, flags=re.I)
    if m:
...
def cache_frames(video_path, cache_dir, nframes=16):
    ...
    for i, arr in enumerate(frames):
        ...
        Image.fromarray(arr).convert("RGB").save(p, quality=95)
        actual.append(str(p))
    return actual
...
def frame_paths_to_pixels(frame_paths, input_size=448):
...
def apply_chat(tokenizer, model, frame_paths, prompt, max_new_tokens=16):
    pixel_values, num_patches_list = frame_paths_to_pixels(frame_paths)
...
def prompted_answer(row, frame_paths, tokenizer, model, aux_tok, aux_model, method):
    base = f"""Question:
{row['question']}
...
"""
    if method == "direct":
        ...
        return apply_chat(tokenizer, model, frame_paths, prompt, max_new_tokens=16), None
    if method == "generic_cot":
        prompt = base + "\nReason about the temporal evidence, then give the final answer as exactly one option letter."
        return apply_chat(tokenizer, model, frame_paths, prompt, max_new_tokens=96), None
    if method == "freeform_cot_matched":
        ...
        return apply_chat(tokenizer, model, frame_paths, prompt, max_new_tokens=140), None

    hint = generate_text_hint(row["question"], row["candidates"], aux_tok, aux_model, method)
    prompt = base + f"\nTemporal hint:\n{hint}\n\nAnswer with exactly one option letter."
    return apply_chat(tokenizer, model, frame_paths, prompt, max_new_tokens=32), hint
...
def yes_probability(question, hypothesis, state, frame_paths, tokenizer, model, yes_ids, no_ids, previous=""):
    pixel_values, num_patches_list = frame_paths_to_pixels(frame_paths)
    pixel_values = pixel_values.to(torch.bfloat16).cuda()
    prefix = "".join([f"Frame{i + 1}: <image>\n" for i in range(len(num_patches_list))])
...
"""
    _, logits = chat_first_token_logits(model, tokenizer, pixel_values, prompt, num_patches_list)
...
def score_option(row, candidate, frame_paths, aux_tok, aux_model, tokenizer, model, yes_ids, no_ids, method, early_exit):
    ...
    if method == "joint":
        ...
        p = yes_probability(row["question"], hyp, joint_state, frame_paths, tokenizer, model, yes_ids, no_ids)
    ...
    for name, state in states:
        ...
        p = yes_probability(row["question"], hyp, state, segments[name], tokenizer, model, yes_ids, no_ids, previous=prev)
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.vl_model, trust_remote_code=True, use_fast=False)
    ...
    with open(args.output, "w", encoding="utf-8") as fout:
        for row in tqdm(rows, desc=f"{args.dataset} {args.method} internvl"):
            ...
            if args.method in PROMPT_METHODS:
                raw, hint = prompted_answer(row, frames, tokenizer, vl_model, aux_tok, aux_model, args.method)
